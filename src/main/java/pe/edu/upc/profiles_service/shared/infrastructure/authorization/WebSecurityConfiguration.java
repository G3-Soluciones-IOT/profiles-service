    ...
    public SecurityFilterChain filterChain(
        ...
        http.authorizeHttpRequests(authorize -> authorize
                ...
                .requestMatchers(HttpMethod.GET, "/**").access((authentication, context) ->
                        hasPermissionOrLegacyJwt((Supplier<Authentication>) authentication, context, legacyJwtIssuer, "read:profiles"))
                .requestMatchers(HttpMethod.HEAD, "/**").access((authentication, context) ->
                        hasPermissionOrLegacyJwt((Supplier<Authentication>) authentication, context, legacyJwtIssuer, "read:profiles"))
                .anyRequest().access((authentication, context) ->
                        hasPermissionOrLegacyJwt((Supplier<Authentication>) authentication, context, legacyJwtIssuer, "write:profiles")));

        http.oauth2ResourceServer(oauth2 -> oauth2.jwt(jwt ->
                jwt.jwtAuthenticationConverter(ProfilesJwtAuthenticationConverter.jwtAuthenticationConverter())));
